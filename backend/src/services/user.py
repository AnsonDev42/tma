...
async def get_access_limits(user: User):
    ...
    response = await supabase.table("access_limits").select("daily_limit").eq("role", user_role).execute()
    if not response.data or len(response.data) == 0:
    ...
    if user_role in ["trial", "pro"]:
        response = await supabase.table("user_subscriptions").select("expires_at").eq("user_id", user_id).execute()
        if not response.data or len(response.data) == 0:
        ...
        if datetime.datetime.fromisoformat(expires_at) < datetime.datetime.now(datetime.UTC):
            ...
            await supabase.table("user_roles").upsert({"role": "free", "user_id": user_id}, ["user_id"])
    ...
    response = await supabase.table("user_access_records").select("*").eq("user_id", user_id).gte("accessed_at", start_of_today.isoformat()).lt("accessed_at", end_of_today.isoformat()).execute()

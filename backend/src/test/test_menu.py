...
async def test_cache_dish_image():
    with TestClient(app) as client:
        ...
        result = (
            ...
            .delete()
            .eq("dish_name", test_dishname)
            .execute()
        )
        build_or_update_cache = await cache_dish_image(test_dishname, ["test", "test2"])
        assert build_or_update_cache in [
            "update cache",
            "build cache",
        ], "no cache is build or update"
...
async def test_prcess_image():
    def process_image_valid_image():
        image = cv2.imencode(".jpeg", np.zeros((1000, 1000, 3), dtype=np.uint8))[
            1
        ].tobytes()
    ...
    def process_image_large_image():
        image = cv2.imencode(".jpeg", np.zeros((3000, 3000, 3), dtype=np.uint8))[
            1
        ].tobytes()
        processed_image, img_height, img_width = process_image(image)
        assert processed_image is not None
    ...
    def process_image_compression():
        image = cv2.imencode(".jpeg", np.zeros((10000, 10000, 3), dtype=np.uint8))[
            1
        ].tobytes()
        processed_image, img_height, img_width = process_image(image)
        assert processed_image is not None
